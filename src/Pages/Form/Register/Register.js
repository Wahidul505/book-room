...
const Register = () => {
    const { setName, setEmail, setPassword, setConfirmPassword, handleRegister, handleGoogleSignIn, error } = useFirebase();
    return (
        <div className='flex flex-col gap-6 w-1/2 mx-auto md:w-1/3 bg-amber-200 rounded-lg p-4'>
            <h1 className='text-center text-3xl'>Register</h1>
            <form onSubmit={handleRegister} className='flex flex-col gap-4'>
                <input onChange={(e) => setName(e.target.value)} className='px-2 py-1' type="text" name="name" required placeholder='Your Name' />
